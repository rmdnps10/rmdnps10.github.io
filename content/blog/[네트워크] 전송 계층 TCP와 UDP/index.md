...
>
> 네트워크 계층에서 IP가 핵심이었다면, 전송 계층에서는 TCP와 UDP가 핵심이다. 이 두 프로토콜은 각각 `신뢰성`과 `속도`라는 서로 다른 목적을 가지고 있으며, 패킷의 최종 목적지인 프로세스까지 데이터를 전달하는 역할을 한다. 본 글에서는 `TCP`와 `UDP`의 동작 원리와 차이점, 그리고 `TCP`의 연결 관리와 제어 메커니즘을 다룬다.
