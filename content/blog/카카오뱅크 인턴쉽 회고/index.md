...
keywords: "카카오뱅크, 인턴십, DevRel, 개발자 경험, DX, 핀테크, 금융, 네이버랩스, 취업, 면접, 프론트엔드, 백엔드, 개발, 어시스턴트"
---

> "금융으로 세상을 바꾸는 기업, 카카오뱅크에서의 6개월간의 인턴쉽을 회고합니다. (+ 네이버랩스 인턴 최종 면접후기)"

![카뱅 춘식이](./index.png "춘식아 안녕")
...
1차와 코딩 테스트를 통과한 뒤 현직자 분과 전화 인터뷰가 있었는데, 현직자 분께서 내 서류를 보고 지금까지 본 서류 중에 제일 잘 쓴 것 같다고 긍정적인 피드백을 주셔서 합격에 대한 기대감이 높아졌다.
...
![네이버랩스 면접 결과](./images/naver-labs.png "아쉬운 결과")
...
그리고 `개발이 필요한 특정 기간동안에는` 위의 일을 진행하며 개발의 비중을 높이면서 업무를 진행했다.
...
(이후에 카카오에서 개최한 24K 해커톤을 참관했는데, 코딩하는 시간을 약 8시간만 제공하고 심사도 AI로 진행하는 등 독특한 방식으로 운영되었다.)
...
뱅커톤에 출품한 모든 작품들에 대한 `아카이빙 웹사이트`를 만들고, 해커톤에 대한 `KPT 회고`를 진행했다. (Keep, Problem, Try)
결과물을 아카이빙하면서 코드를 살펴본 결과, 지금까지 내가 본 학생들로만 이루어진 해커톤에서 나온 결과물과는 확실히 수준 차이가 많이 났다. 카카오뱅크 개발자들의 프로페셔널리즘을 여실히 느낄 수 있었다.
...
글을 세상 밖으로 내보내는 과정은 정말 리소스가 많이 드는 작업이다. 이때의 경험 덕분에 지금 기술 블로그에 글을 쓸 때도 습관적으로 퇴고의 과정을 거치고 있다.
...
![API 명세서](./images/api-docs.png "API 명세서")
...
카카오뱅크 같은 기술 조직에서 **AI 활용 전략**에 대해 어떤 고민을 하고 있는지 엿볼 수 있었다.
...
기술조직의 채용 과정을 옆에서 지켜보면서는 개발자로서 어떤 역량을 어필해야 하고, 반대로 어떤 부분들이 감점 요소가 될 수 있는지에 대한 실질적인 인사이트도 얻었다.
