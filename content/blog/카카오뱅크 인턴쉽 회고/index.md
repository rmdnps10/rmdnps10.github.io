...
전화 합격 약 한 달 뒤에 네이버 사옥에서으 최종 대면 면접을 통보받았다. 사전에 10분 정도 직무와 관련한 경험들에 대한 소개를 하는 presentation을 준비하라고 하셨고, 발표자료를 미리 HR 담당자에게 송부했다.

듣던 대로 정말 네이버 사옥에서는 음식 배달 등을 위해 정말 로봇들이 자유롭게 이동하는 모습을 볼 수 있었다.
면접 전에 HR 담당자 분과 10분 정도 면접, 직무에 대한 소개를 받았는데... **나에게 마음을 단단히 먹으라고 하셨다.** 워낙 면접 난이도가 빡센 걸로 악명이 높다.. 면접은 일대다(**면접자 1, 면접관 4**!!)로 1-2시간 동안 진행될 거라고 해서, 긴장을 잔뜩 한 채로 면접에 들어갔다.
...
동시에 카카오뱅크 개발 어시스턴트에도 지원했었다. 당시 우대사항 및 요구되는 역량이
...
이었는데 당시에 개발 커뮤니티 운영진(멋사)과 스타트업 인턴을 하면서 했던 테크니컬 라이팅, 교육 컨텐츠 제작 경험들을 서류에서 어필했다.
...
3월 달에 카카오뱅크 최초 사내 해커톤인 뱅커톤을 개최했다. 우리 팀에서 end-to-end로 뱅커톤을 지원했는데, 그래서 처음 입사한 1월 말에는 뱅커톤을 준비하는 회의를 내부적으로 또는 타부서와 매일 하곤 했다.
...
모든 개발 애셋은 React + TypeScript + Vite 스택으로 구성했고, 실시간 점수 업데이트를 위해 WebSocket 연결을 활용했다!
...
확실히 그동안 학생들로만 이루어진 해커톤에서 나온 결과물과는 수준 차이가 많이 났다. 개발자의 프로페셜리즘을 또 한번 느낄 수 있었다.

#### 행사 이후

뱅커톤 행사 이후에는 오히려 행사 뒷작업을 하느라 바빴다.
뱅커톤에 출품한 모든 작품들에 대한 아카이빙 웹사이트를 만들고, 해커톤에 대한 KPT 회고를 진행했다. (Keep, Probelm, Try)
...
인턴십 기간 동안 총 5회차의 DevInvite 행사를 기획하고 운영했다. 높은 퀄리티의 강의를 세션 때 마다 들을 수 있어서, 개발에 관심있는
내 입장에서도 굉장히 흥미롭게 들었다. 특히 당근마켓 천재환 님의 강연이 인상이 깊었다. ML Engineer 로서
...
카카오뱅크에서는 사내 기술 컨퍼런스, 코드 러너를 매년 개최한다. 어떻게보면 우리 팀의 1년 중 가장 큰 과제이기도 한데..
행사 진행을 하는데 있어서 나는 개발 산출물을 내는 역할을 맡았다.

작년 코드러너에서는 운영진이 참석자들의 컨퍼런스 출석을 체크하기 위해 QR 리더기를 통해 체크를 했는데, 이번에는 참가자들에게 핸드북 링크를 제공하여 아래의 기능들을 추가하고자 하는 목표를 가지고 설계를 시작했다.
...
참고로 코드러너에서는 기술조직 구성원들이 모두 참여해 매년 예선을 거친 뒤 현장에서 결선을 걸치는 단체 게임을 진행한다!
지금까지 했던 단체게임으로는 돈 개수 세기, 타자빨리치기... 가 있었다.

인턴 마지막 2달의 시간동안 총 6개의 프로그램을 개발했다.

#### 1. 코드러너 메인서버 개발

- **Backend Stack**: FastAPI + PostgreSQL + Docker Container 구성
- **보안**: 기술팀과 협의한 AES-256 대칭키 암호화를 통해 사용자 ID를 안전하게 저장
- **API 설계**: RESTful API 설계 원칙을 따라 출석 체크, 베팅, 럭키드로우 응모권 관리 엔드포인트 구현
- **데이터베이스**: PostgreSQL의 ACID 특성을 활용하여 동시성 이슈 방지 및 트랜잭션 무결성 보장

FastAPI로 제대로 개발해보는 건 처음이어서, Pydantic 모델링과 의존성 주입 패턴을 익히는 데도 시간이 걸렸다. 

#### 2. 핸드북 (참여자 클라이언트 웹)

- **Navigation**: Stackflow 라이브러리를 통해 네이티브 앱과 유사한 자연스러운 페이지 전환 구현
- **UX/UI**: Intersection Observer API를 활용하여 스크롤 기반 페럴럭스 효과와 요소별 지연 애니메이션 적용
- **성능 최적화**: TanStack Query(React Query)의 staleTime, cacheTime 설정을 통한 서버 요청 최소화

특히 동시 접속자가 몰릴 때를 대비해 데이터 prefetching과 background refetch 전략을 세심하게 조정했던 기억이 난다.

#### 3. 럭키드로우


- **Design System**: 카카오뱅크 BI 가이드라인을 준수하여 브랜드 일관성 유지
- **벡터 그래픽**: SVG 포맷으로 추첨 머신을 구현하여 해상도 독립적 렌더링 및 애니메이션 최적화

CSS3 transform과 GSAP 라이브러리를 조합하여 부드러운 추첨 효과 구현했다.
...
- **게임 컨셉**: `김카뱅의 환전소`라는 카카오뱅크 아이덴티티를 살린 보드게임 '카지노' 룰 기반 디지털 게임
- **아키텍처**: WebSocket 기반 실시간 양방향 통신으로 Supervisor Admin과 Display 화면 간 상태 동기화
- **네트워크**: 동일 네트워크 환경 가정 하에 Socket.io를 통한 low-latency 통신 구현
- **성능**: 10ms 간격의 고빈도 데이터 전송을 위한 debouncing과 throttling 적용
- **최적화**: React.memo, useMemo, useCallback을 활용한 불필요한 리렌더링 방지 및 Virtual DOM 최적화

실시간 동기화가 핵심이었기 때문에 WebSocket 연결 안정성과 에러 핸들링에 특히 신경을 많이 썼다.

#### 5. 메인 디스플레이 화면

- **3D Graphics**: Three.js를 활용한 3D 씬 렌더링 및 카메라 애니메이션 구현
- **슬라이드 시스템**: Swiper.js 라이브러리 기반의 자동 슬라이드 전환 및 인터랙티브 네비게이션
- **관리 시스템**: 별도 Admin 패널을 통한 실시간 메시지 브로드캐스팅 및 타이머 제어 기능
- **반응형**: CSS Grid와 Flexbox를 활용한 다양한 디스플레이 해상도 대응

#### 6. 운영진 QR 스캔 전용 웹앱

npm 생태계의 QR 스캔 라이브러리들을 실제 테스트하여 성능, 호환성, 사용성 기준으로 최적 선택했다.
가장 마지막에 개발한 프로젝트여서 시간이 촉박했지만, 핵심 기능에 집중하여 안정적인 QR 스캔 환경을 구축하는 데 성공했다.

### 느낀 점

팀 특성상 기술 조직에 있는 크루들과 이야기할 기회가 많아서, 현업 개발자들의 시각에서 내가 부족한 부분과 앞으로 필요한 역량에 대한 구체적인 조언을 들을 수 있었다.

특히 **AI 활용 전략**에 대한 논의가 인상 깊었다. 단순히 코드 생성을 위한 도구가 아니라, 문제 해결의 사고 과정을 확장하고 학습 효율을 높이는 방향으로 AI를 활용하는 방법에 대해 많은 인사이트를 얻었다. 또한 **기술 트렌드와 학습 로드맵**에 대해서도 실무진들의 경험담을 통해 어떤 기술을 언제, 어떻게 학습해야 효과적인지에 대한 현실적인 조언을 받을 수 있었다.
