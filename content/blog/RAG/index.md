---
title: "RAG의 핵심 개념과 원리를 실습과 함께 알아봅니다."
slug: "rag-core-concept"
date: "2025-10-01"
...
기존 언어 모델이 사전 학습된 지식에만 의존하는 반면, `RAG`는 `외부 데이터베이스`에서 관련 정보를 실시간으로 검색하여 더 정확하고 최신의 답변을 생성한다.
...
`ChatGPT`와 같은 **대량의 데이터를 학습한 대규모 언어 모델**도 한계가 있다.

- 학습 이후에 새로 나온 데이터에 대해서는 답변이 어려움 (`Knowldege Cutoff`)
- 사실이 아닌 내용을 그럴듯하게 생성하는 문제 (`Hallucination`)
- 학습하지 않은 특정 도메인 데이터들에 대한 제한적 지식 (`Training Data Cutoff`)

이러한 문제점을 해결하는 기법이 `RAG` 인데, `RAG`는 **실시간으로 부족한 데이터를 주입**하기 위해 답변 전에 외부 문서에 대한 맥락을 주입시켜 준다.
...
> **💡 chunk 사이즈에 따른 성능**
> 
> - `chunk size`가 작을수록 정밀도가 올라가나, 문맥이 잘리고 chunk 개수가 많아져서 검색 속도가 느림
> - `chunk size`가 클수록 문맥 보존에는 유리하나 의미가 희석되어 정밀도가 낮아지고 관계 없는 정보가 함께 포함될 수 있음

#### 쿼리 벡터화 및 관련 정보 추출

사용자의 **질문을 벡터화**하고, `벡터 DB`를 대상으로 `유사도 검색`을 사용하여 가장 관련성이 높은 상위 K개의 항목을 추출한다.

##### 벡터 유사도 검색

임베딩을 통해 저장된 데이터는 **벡터 간의 거리를 측정**하여 얼마나 유사한지를 평가한다. 
유사도 측정 방식에는 아래와 같은 방법들이 있다.
...
`외부 지식`을 검색해 `기존 모델`에 전달하고 그를 바탕으로 응답을 생성하는 방식이다.

#### Fine-tuning 

`기존 모델`에 `새로운 데이터`를 학습시켜, 모델 자체를 바꾸는 방식이다.
...
`Vector DB`는 데이터를 `벡터 형태로 저` 하고, **벡터 간 유사도를 기반으로 검색**하는 데이터베이스다.
기존의 관계형 DB가 행과 열의 2차원 형식으로 데이터를 표현하는 반면, `Vector DB`는 **다차원 공간으로 개념을 확장**한다.

#### Vector DB가 필요한 이유

- 텍스트를 AI가 이해 가능한 형태인 벡터로 변환하여 저장
